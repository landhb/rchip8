...
use crate::cpu::{FLAG_REGISTER, MEM_SIZE, TXT_OFFSET};
use anyhow::Result;
...
impl Debugger for crate::cpu::Cpu {
    ...
    fn disas(&self, len: usize) {
        ...
        let text_section: &[u8] =
            &self.memory[self.program_counter - 2..self.program_counter + len];
        for (i, v) in text_section.chunks_exact(2).enumerate() {
            ...
            match nums {
                [0, 0, 0xE, 0] => {
                    self.print_instruction(i, "CLS", comments);
                }
                [0, 0, 0xE, 0xE] => {
                    self.print_instruction(i, "RET", comments);
                }
                [0, n, _, _] => {
                    self.print_instruction(i, &format!("SYS 0x{:X}{:X}", n, v[1]), comments);
                }
                [1, n, _, _] => {
                    self.print_instruction(i, &format!("JP 0x{:X}{:X}", n, v[1]), comments);
                }
                [2, n, _, _] => {
                    self.print_instruction(i, &format!("CALL 0x{:X}{:X}", n, v[1]), comments);
                }
                [3, x, _, _] => {
                    self.print_instruction(i, &format!("SE V{:X}, 0x{:X}", x, v[1]), comments);
                } // skip instruction if Vx == kk
                [4, x, _, _] => {
                    self.print_instruction(i, &format!("SNE V{:X}, 0x{:X}", x, v[1]), comments);
                } // skip instruction if Vx != kk
                [5, x, y, _] => {
                    self.print_instruction(i, &format!("SE V{:X}, V{:X}", x, y), comments);
                } // skip instruction if Vx == Vy
                [6, x, _, _] => {
                    self.print_instruction(i, &format!("LD V{:X}, {:X}", x, v[1]), comments);
                } // load kk into Vx
                [7, x, _, _] => {
                    self.print_instruction(i, &format!("ADD V{:X}, {:X}", x, v[1]), comments);
                } // add kk to Vx
                [8, x, y, 0] => {
                    self.print_instruction(i, &format!("LD V{:X}, V{:X}", x, y), comments);
                } // set Vx = Vy
                [8, x, y, 1] => {
                    self.print_instruction(i, &format!("OR V{:X}, V{:X}", x, y), comments);
                } // Vx = Vx OR Vy
                [8, x, y, 2] => {
                    self.print_instruction(i, &format!("AND V{:X}, V{:X}", x, y), comments);
                } // Vx = Vx AND Vy
                [8, x, y, 3] => {
                    self.print_instruction(i, &format!("XOR V{:X}, V{:X}", x, y), comments);
                } // Vx = Vx XOR Vy
                [8, x, y, 4] => {
                    self.print_instruction(i, &format!("ADD V{:X}, V{:X}", x, y), comments);
                } // Vx = Vx + Vy, VF = carry
                [8, x, y, 5] => {
                    self.print_instruction(i, &format!("SUB V{:X}, V{:X}", x, y), comments);
                } // Vx = Vx - Vy, set VF = NOT borrow.
                [8, x, y, 6] => {
                    self.print_instruction(i, &format!("SHR V{:X}, {{,V{:X}}}", x, y), comments);
                } // shift right
                [8, x, y, 7] => {
                    self.print_instruction(i, &format!("SUBN V{:X}, V{:X}", x, y), comments);
                }
                [8, x, y, 0xE] => {
                    self.print_instruction(i, &format!("SHL V{:X}, V{:X}", x, y), comments);
                } //shift left
                [9, x, y, 0] => {
                    self.print_instruction(i, &format!("SNE V{:X}, V{:X}", x, y), comments);
                } // same as before?
                [0xA, n, _, _] => {
                    self.print_instruction(i, &format!("LD I, 0x{:X}{:X}", n, v[1]), comments);
                }
                [0xB, n, _, _] => {
                    self.print_instruction(i, &format!("JP V0, 0x{:X}{:X}", n, v[1]), comments);
                }
                [0xC, x, _, _] => {
                    self.print_instruction(i, &format!("JP V{:X}, 0x{:X}", x, v[1]), comments);
                }
                [0xD, x, y, n] => {
                ...
                [0xE, x, 9, 0xE] => {
                    self.print_instruction(i, &format!("SKP V{:X}", x), comments);
                }
                [0xE, x, 0xA, 1] => {
                    self.print_instruction(i, &format!("SKNP V{:X}", x), comments);
                }
                [0xF, x, 0, 7] => {
                    self.print_instruction(i, &format!("LD V{:X}, DT", x), comments);
                }
                [0xF, x, 0, 0xA] => {
                    self.print_instruction(i, &format!("LD V{:X}, K", x), comments);
                }
                [0xF, x, 1, 5] => {
                    self.print_instruction(i, &format!("LD DT, V{:X}", x), comments);
                }
                [0xF, x, 1, 8] => {
                    self.print_instruction(i, &format!("LD ST, V{:X}", x), comments);
                }
                [0xF, x, 1, 0xE] => {
                    self.print_instruction(i, &format!("ADD I, V{:X}", x), comments);
                }
                [0xF, x, 2, 9] => {
                    self.print_instruction(i, &format!("LD F, V{:X}", x), comments);
                }
                [0xF, x, 3, 3] => {
                    self.print_instruction(i, &format!("LD B, V{:X}", x), comments);
                }
                [0xF, x, 5, 5] => {
                    self.print_instruction(i, &format!("LD [I], V{:X}", x), comments);
                }
                [0xF, x, 6, 5] => {
                    self.print_instruction(i, &format!("LD V{:X}, [I]", x), comments);
                }
                [_, _, _, _] => {
                    println!("{:X}", var);
                    unimplemented!();
        ...
        while let ReadResult::Input(mut cmd) = reader.read_line()? {
            ...
            match cmd.as_str().trim_end_matches('\r').trim_end_matches('\n') {
                ...
                x if x.starts_with('x') => {
                    ...
                    println!(
                        "{:?}",
                        &self.memory[addr..addr + amount].to_vec().hex_dump()
                    );
                }
                ...
                v => {
                    println!("[-] unrecognized debugger command: {} (try \"help\")", v);
                    continue;
                }
            }
